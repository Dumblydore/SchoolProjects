...
//            remote: "/search/state?state=Michigan",
        prefetch: {
            url: '../search/state?state=Michigan'
        }
    });
    ...
    myTypeahead.on('typeahead:selected',function(obj,datum,name){
        ...
        $.ajax ({
                type: "GET",
                url: "../search/politician?state=Michigan&politician=" +  datum['politicianName'],
                dataType: 'json'
            }
        ).then(function(data)
            {
                var photo = data[0].politicianPhoto != null ? data[0].politicianPhoto : "../assets/default-image.png"
